---
sidebar_position: 3
title: "The Glue: Why Middleware Exists"
---
...
```mermaid
graph TB
    subgraph "Sensors (Publishers)"
        M1["Motor 1<br/>publishes<br/>position"]
        M2["Motor 2<br/>publishes<br/>position"]
        IMU["IMU<br/>publishes<br/>orientation"]
        LIDAR["LIDAR<br/>publishes<br/>point cloud"]
    end

    subgraph "Middleware (Message Bus)"
        BUS["ROS 2 Middleware<br/>Topic: /motor1/position<br/>Topic: /motor2/position<br/>Topic: /imu/data<br/>Topic: /lidar/scan"]
    end

    subgraph "Consumers (Subscribers)"
        Main["Main Controller<br/>subscribes to all"]
        Logger["Data Logger<br/>subscribes to sensor topics"]
        Monitor["Health Monitor<br/>subscribes to motor data"]
    end
    ...
    style IMU fill:#e3f2fd
    style LIDAR fill:#e3f2fd
...
graph TB
    subgraph "ROS 2 Nodes"
        IMU["IMU Node<br/>publisher"]
        Motor["Motor Controller<br/>subscriber + publisher"]
        Main["Main Controller<br/>subscriber + service caller"]
    end

    subgraph "ROS 2 Topics"
        T1["Topic: /imu/data"]
        T2["Topic: /motor/cmd_vel"]
    end

    subgraph "ROS 2 Services"
        S1["Service: /move_to_goal"]
    end
...
```mermaid
graph LR
    Real["Real World<br/>(physical sensors<br/>with all their messiness)"]

    Middleware["Middleware<br/>(standardizes,<br/>buffers,<br/>routes)"]

    Code["Your Code<br/>(clean interfaces,<br/>predictable data)"]

    Real -->|raw data| Middleware
    Middleware -->|standard messages| Code
```
