...
```mermaid
graph TB
    subgraph ChatGPT["ChatGPT: Software AI"]
        Q["Your Question"]
        N["Neural Network<br/>(silicon)"]
        A["Answer<br/>Latency: ~500ms"]
        Q --> N --> A
        style N fill:#e1f5ff
    end

    subgraph Robot["Walking Robot: Embodied AI"]
        C["Camera"]
        P["Processing<br/>(onboard or cloud)"]
        M["Motor Command"]
        F["Motor Feedback<br/>Latency: 100-500ms"]
        E["Environment<br/>Responds"]
        C --> P --> M --> F --> E --> C
        style M fill:#ffecb3
        style F fill:#ffecb3
    end

    ChatGPT -.-> |No feedback loop| Robot
```
...
- You get a response in <2 seconds
...
**Previous:** [Chapter Overview →](./index.mdx) | **Next:** [Lesson 1.2: Embodied Intelligence →](./02-embodied-intelligence.mdx)
